    ...
    <section>
        <Element name="features">
            <div className='container'>
                ...
                md:g7 max-md:border-none max-md:roundd-none max-md:gap-3'>
                    {features.map(({id, icon, caption, title, text,button}) => (
                        <div key={id} className='relative z-2 md:px-10 px-5 md:pb-10 pb-5 flex-50 max-md:g7
                        max-md:border-2 max-md:border-s3 max-md:rounded-3xl max-md:flex-320'>
                            ...
                            <p className='caption mv-5 max-md:mb-6'>{caption}</p>
                    ...
                    <ul className='relative flex justify-around flex-grow px-[5%] border-2 border-s3 rounded-7zl max-md:hidden'>
                        <div className='absolute bg-s3/20 top-[38%] left-0 right-0 w-full h-[1px] z-10'/>
                        {details.map(({id, icon, title}) => (
                            <li key={id} className='relative pt-16 px-4 pb-14 '>
                                <div className='absolute top-8 bottom-0 left-1/2 bg-s3/20 w-[1px] h-full z-10' />
                                <div className='flex items-center justify-center mx-auto mb-3 border-2 border-s2 rounded-full
                                hover:border-s4 transition-all duration-500 shadow-500 size-20'>
                                    <img src={icon} alt={title} className='size-17/20 object-contain z-2' />
                                </div>
